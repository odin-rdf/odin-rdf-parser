// Package iri implements RFC 3986 section 5 reference resolution for the
// Turtle-family parsers. Internal to odin-rdf-parser.
//
// Resolution is lexical and byte-level: no percent-decoding, no case
// normalization (scheme-level validation only is a vision non-goal). The
// resolved IRI is built in a caller-owned scratch buffer and interned, so
// steady-state resolution allocates only for IRIs the intern table has
// not seen (ADR RDF-A-0001).
package iri

import rdf "../.."

// Components is an IRI reference split per RFC 3986 appendix B. Slices
// borrow from the parsed string; the has_* flags distinguish an absent
// component from an empty one (the distinction matters in resolution).
Components :: struct {
...
// parse splits an IRI reference into components. It cannot fail: every
// string is a syntactically valid reference under the appendix B
// grammar's totality (garbage lands in the path).
parse :: proc(s: string) -> (c: Components) {
	rest := s
...
// Scratch is caller-owned working memory for resolve; its capacity is
// retained across calls, so steady-state resolution does not allocate.
Scratch :: struct {
...
// resolve transforms reference ref against base per RFC 3986 §5.2.2
...
@(private)
prefix :: proc(s: string, p: string) -> bool {
